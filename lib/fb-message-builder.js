...
class Generic extends FacebookTemplate {
  ...
  addUrl(url, messenger_extensions = true, webview_height_ratio = 'full') {
    if (!url)
    ...
    this.getLastBubble()['default_action'] = {
      type: 'web_url',
      url,
  ...
  addButton(title, value, isWebview = false, heightRatio = 'tall', isSharable = false) {
    ...
    if (isWebview) {
      ...
      if (isSharable) {
        button.webview_share_button = true;
      }
    }
